...
def create_pdf(symboltable="symboltable", pagefiles: list = None, dpi_map_path=None):
    # def create_pdf(symboltable: str = "symboltable", pagefiles: list = None):
    """Load DPI Map"""
    dpi_map = {}
    if dpi_map_path:
        try:
            with open(dpi_map_path) as f:
                dpi_map = json.load(f)
        except Exception as e:
            sys.stderr.write(f"Error reading DPI map: {e}\n")
    ...
    for i, p in enumerate(pagefiles):
        ...
        try:
            idx = int(Path(p).name.split(".")[-1])  # For files like output.0
        except ValueError:
            idx = i

        xres = xres if xres else dpi_map.get(str(idx), {}).get("x", dpi)
        yres = yres if yres else dpi_map.get(str(idx), {}).get("y", dpi)
...
def parse_args():
    ...
    parser.add_argument("-s", action="store_true", help="Standalone mode")
    parser.add_argument(
        "--dpi-map", help="Path to JSON file with DPI info", default=None
    )
    ...
    if args.basename:
        sym = f"{args.basename}.sym"
        pages = glob.glob(f"{args.basename}.[0-9]*")
    else:
        sym = "symboltable"
        pages = glob.glob("page-*")

    validate_file_exists(sym, sys.argv[0], f"symbol table '{sym}' not found!")
    if not pages:
        usage(sys.argv[0], "no pages found!")

    return sym, pages, args.dpi_map
...
if __name__ == "__main__":
    sym, pages, dpi_map_path = parse_args()
